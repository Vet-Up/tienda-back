...
class UserServiceImplTest {
    ...
    class CreateUserTests {
        ...
        void testCreateUserEmailExists() {
            ...
            when(userRepository.getClientByEmail(userDtoToCreate.email())).thenReturn(Optional.of(existingUser));

            BusinessException exception = assertThrows(BusinessException.class,
                    () -> userServiceImpl.createUser(userDtoToCreate));

            assertEquals("User with email " + userDtoToCreate.email() + " already exists", exception.getMessage());
        }
    }
    ...
    class UpdateUserTests {
        ...
        void testUpdateUser() {
            ...
            when(userRepository.getClientByEmail(userDtoToUpdate.email())).thenReturn(Optional.of(existingUserEntity));
        ...
        void testUpdateUserEmailConflict() {
            ...
            when(userRepository.getClientById(userIdToUpdate)).thenReturn(Optional.of(existingUserEntity));
            when(userRepository.getClientByEmail(userDtoToUpdate.email())).thenReturn(Optional.of(anotherUserEntity));

            BusinessException exception = assertThrows(BusinessException.class,
                    () -> userServiceImpl.updateUser(userDtoToUpdate));

            assertEquals("Another user with email " + userDtoToUpdate.email() + " already exists", exception.getMessage());
        }
    }
